...
test('it strictly clears pre-existing global libxml errors and restores internal state', function () use ($tinyPng): void {
    ...
    libxml_use_internal_errors($originalState);
});

test('it strictly clears libxml errors generated by parsing malformed svg content', function () use ($tinyPng): void {
    $originalState = libxml_use_internal_errors(true);
    libxml_clear_errors();

    $merger = new SvgMerger('<invalid><unclosed-tag>', $tinyPng, 0.2);

    try {
        $merger->merge();
    } catch (ImageMergeException) {
    }
