...
test('it throws exception for invalid percentages', function () use ($tinyPng) {
    expect(fn () => new RasterMerger($tinyPng, $tinyPng, 0))
        ->toThrow(InvalidArgumentException::class, '$percentage must be between 0 and 1');

    expect(fn () => new RasterMerger($tinyPng, $tinyPng, 1))
        ->toThrow(InvalidArgumentException::class, '$percentage must be between 0 and 1');
});

test('it throws exception for unsupported formats', function () use ($tinyPng) {
    expect(fn () => (new RasterMerger($tinyPng, $tinyPng, 0.2))->setFormat(Format::SVG))
        ->toThrow(InvalidArgumentException::class, 'RasterMerger only supports "png" or "webp" formats.');
});

test('it properly propagates InvalidArgumentException for invalid image data', function () use ($tinyPng) {
    expect(fn () => new RasterMerger('not-an-image', $tinyPng, 0.2))
...
test('it successfully merges and sets formatting for WEBP', function () use ($tinyPng) {
    $result = (new RasterMerger($tinyPng, $tinyPng, 0.2))->setFormat(Format::WEBP)->merge();

    expect($result)->toBeString()->toContain('WEBP');
});

it('throws exception if transparent color cannot be created', function () use ($tinyPng) {
    global $mockImageColorAllocateAlpha;
    $mockImageColorAllocateAlpha = false;

    expect(fn () => (new RasterMerger($tinyPng, $tinyPng, 0.2))->merge())
...
it('throws exception if image canvas cannot be created', function () use ($tinyPng) {
...
it('throws exception if imagefill fails', function () use ($tinyPng) {
...
it('throws exception if imagecopy fails', function () use ($tinyPng) {
...
it('throws exception if imagecopyresampled fails', function () use ($tinyPng) {
...
it('throws exception if imagesavealpha fails', function () use ($tinyPng) {
...
it('throws exception if output buffer capture fails', function () use ($tinyPng) {
...
it('throws exception if image creation fails returning false', function () {
    ...
    expect(fn () => new Image('valid string but mock fails'))
        ->toThrow(InvalidArgumentException::class, 'Invalid image data provided to Image.');
})->after(function () {
    global $mockImageCreateFromString;
...
it('preserves full alpha transparency on the canvas to kill alpha blending and savealpha mutants', function () {
    $canvas = \imagecreatetruecolor(100, 100);
...
it('throws exception when output content is an empty string', function () use ($tinyPng) {
